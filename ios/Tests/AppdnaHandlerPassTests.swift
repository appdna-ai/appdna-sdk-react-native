...
final class AppdnaHandlerPassTests: XCTestCase {
    ...
    private func writeEvidence(_ methods: [String: String]) throws {
        ...
        let out = root.appendingPathComponent("packages/appdna-sdk-react-native/handler-pass-ios.json")
    ...
    /// The test bundle runs from DerivedData, so `#filePath` — this file, in the repo — is the anchor.
    static func repoRoot() -> URL? {
        var dir = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
        for _ in 0..<12 {
            if FileManager.default.fileExists(atPath: dir.appendingPathComponent("packages/sdk-shared-fixtures").path) {
                return dir
            }
